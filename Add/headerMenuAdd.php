<?php include("../db.php");
$message="";
if(isset($_POST['name']) && isset($_POST['slag']) && isset($_POST['status'])){
  $name=$_POST['name'];
  $slag=$_POST['slag'];
  $status=$_POST['status'];
  $sql='SELECT * FROM `headerMenu` WHERE `menu`="'.$name.'" OR `slag`="'.$slag.'"';
  $result=mysqli_query($conn,$sql);
  if(mysqli_num_rows($result)<=0){
    if($name!="" && $slag!="" && $status!="" ){
      $sql2='INSERT INTO `headerMenu`(`id`, `menu`, `slag`, `status`) VALUES (NULL,"'.$name.'","'.$slag.'","'.$status.'")';
...
<form class="wd30" method="post">
  ...
  <option >Choose</option>
    ...
    $name2="";
    $write="";
    $sql='SELECT * FROM `translation` WHERE `name` LIKE "slag%"';
    $result=mysqli_query($conn,$sql);
    if(mysqli_num_rows($result)>0){
  ...
  <div class="mb-3">
    <label for="exampleInputPassword1" class="form-label">Slag</label>
    <input type="text" class="form-control" id="exampleInputPassword1" name="slag">
  </div>
  <div class="mb-3">
  ...
  <div class="text-center col"><button type="submit" class="btn btn-primary">Submit</button>
   <button type="submit" class="btn btn-danger"><a href="../headerMenu.php" class="text-light text-decoration-none">Go Back</a></button></div>
</form>
