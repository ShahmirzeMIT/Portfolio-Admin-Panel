...
include('../db.php');
$message="";
if(isset($_POST['lang'])  &&isset($_POST['name']) && isset($_POST['status']) ){
$lang=$_POST['lang'];
$name=$_POST['name'];
$status=$_POST['status'];
	if($lang!==""  && $name!=="" && $status!==""){
		$sql='INSERT INTO `menu` (`id`, `lang`, `name`, `status`) VALUES (NULL,"'.$lang.'","'.$name.'","'.$status.'")';
...
<form class="wd30" method="post">
  ...
  <div class="text-center col"><button type="submit" class="btn btn-primary">Submit</button>
   <button type="submit" class="btn btn-danger"><a href="../menu.php" class="text-light text-decoration-none">Go Back</a></button></div>
</form>
