<?php include("../db.php");
$message="";
if(isset($_POST['lang']) && isset($_POST['workName']) && isset($_POST['year']) && isset($_POST['text'])){		
	$lang=strtolower($_POST['lang']);
	$workName=$_POST['workName'];
	$year=$_POST['year'];
	$text=$_POST['text'];
	$sql='SELECT * FROM `experience` WHERE `lang`="'.$lang.'"';
	$result=mysqli_query($conn,$sql);
	if(mysqli_num_rows($result)<=0){
		if($lang!=="" && $workName!=="" && $year!=="" && $text!==""){
			$sql='INSERT INTO `experience`(`id`, `lang`, `workName`, `year`, `text`) VALUES (NULL,"'.$lang.'","'.$workName.'","'.$year.'","'.$text.'")';
			$result=mysqli_query($conn,$sql);
...
<form class="wd30" method="post">
  ...
  <div class="mb-3">
    <label for="exampleInputPassword1" class="form-label">WorkName</label>
    <input type="text" class="form-control" id="exampleInputPassword1" name="workName">
  </div>
  <div class="mb-3 ">
