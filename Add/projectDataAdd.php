...
include('../db.php');
$message="";
if(isset($_POST['lang']) && isset($_POST['target']) &&isset($_POST['text']) && isset($_POST['status']) ){
$lang=$_POST['lang'];
$target=$_POST['target'];
$text=$_POST['text'];
$status=$_POST['status'];
	...
	if($lang!=="" && $target!=="" && $text!=="" && $status!==""){
		$sql='SELECT * FROM `projectdata` WHERE `lang`="'.$lang.'"';
		$result=mysqli_query($conn,$sql);
		if(mysqli_num_rows($result)<=0){
			$sql='INSERT INTO `projectdata`(`id`, `lang`, `target`, `text`, `status`) VALUES (NULL,"'.$lang.'","'.$target.'","'.$text.'","'.$status.'")';
			$result=mysqli_query($conn,$sql);
		}
...
<form class="wd30" method="post">
  ...
  <div class="text-center col"><button type="submit" class="btn btn-primary">Submit</button> <button type="submit" class="btn btn-danger"><a href="../projectData.php" class="text-light text-decoration-none">Go Back</a></button></div>
</form>
