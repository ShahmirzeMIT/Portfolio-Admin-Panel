<?php include("../db.php");
$message="";
if(isset($_POST['name']) && isset($_FILES['file']) && isset($_POST['text']) && isset($_POST['workplace'])  && $text=$_POST['link']){
  $name2=$_POST['name'];
  $text=$_POST['text'];
  $link=$_POST['link'];
  $workplace=$_POST['workplace'];
  $sql='SELECT * FROM `slide` WHERE `name`="'.$name2.'" OR `text`="'.$text.'" OR `workplace`="'.$workplace.'" OR `link`="'.$link.'"' ;
  $result=mysqli_query($conn,$sql);
  if(mysqli_num_rows($result)<=0){
    if($name2!="" && $text!=""  && $workplace!=""  && $link!=""){
	...
	if ($ok)
	 move_uploaded_file($temp, $path);
      $sql2='INSERT INTO `slide`(`id`, `name`, `src`, `text`, `workplace`, `link`) VALUES (NULL,"'.$name2.'","'.$name.'","'.$text.'","'.$workplace.'","'.$link.'")';
