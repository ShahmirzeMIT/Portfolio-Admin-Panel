<?php include("../db.php");
$message="";
if(isset($_POST['name']) && isset($_FILES['file']) && isset($_POST['text']) && isset($_POST['workplace'])){
  $name2=$_POST['name'];
  $text=$_POST['text'];
  $workplace=$_POST['workplace'];
  $sql='SELECT * FROM `slide` WHERE `name`="'.$name2.'" OR `text`="'.$text.'" OR `workplace`="'.$workplace.'"';
  $result=mysqli_query($conn,$sql);
  if(mysqli_num_rows($result)<=0){
    if($name2!="" && $text!=""  && $workplace!="" ){
	...
	if ($ok)
	 move_uploaded_file($temp, $path);
      $sql2='INSERT INTO `slide`(`id`, `name`, `src`, `text`, `workplace`) VALUES (NULL,"'.$name2.'","'.$name.'","'.$text.'","'.$workplace.'")';
