...
include('../db.php');
$sentId=$_GET['id'];
$sql='SELECT * FROM `project` WHERE `id`="'.$sentId.'"';
...
<form class="wd30" method="post">
  ...
  <div class="text-center col">
	...
	<button type="submit" class="btn btn-danger"><a href="../project.php" class="text-light text-decoration-none">Go Back</a></button></div>
</form>
<?php
	if(isset($_POST['submit'])){
		...
		$text=$_POST['text'];
		$status=$_POST['status'];
		$sql='UPDATE `project` SET `lang`="'.$lang.'",`work`="'.$work.'",`text`="'.$text.'",`status`="'.$status.'" WHERE `id`="'.$id.'"';
		$result=mysqli_query($conn,$sql);
		if($result>0){
