...
include('../db.php');
$sentId=$_GET['id'];
$sql='SELECT * FROM `projectdata` WHERE `id`="'.$sentId.'"';
...
<?php
	if(isset($_POST['submit'])){
		...
		$text=$_POST['text'];
		$status=$_POST['status'];
		$sql='UPDATE `projectdata` SET `lang`="'.$lang.'",`target`="'.$target.'",`text`="'.$text.'",`status`="'.$status.'" WHERE `id`="'.$id.'"';
		$result=mysqli_query($conn,$sql);
		if($result>0){
		 	header("Location: ../projectData.php");
