...
include('../db.php');
$sentId=$_GET['id'];
 $sql='SELECT * FROM `hire` WHERE `id`="'.$sentId.'"';
...
<form class="wd30" method="post">
  ...
  <div class="mb-3 ">
    ...
    <input type="text" class="form-control" id="exampleInputPassword1"  value="<?=$hire?>" name="hire">
  </div>
...
<?php
	if(isset($_POST['submit'])){
		...
		$hire=$_POST['hire'];
		$status=$_POST['status'];
		$sql='UPDATE `hire` SET `lang`="'.$lang.'",`target`="'.$target.'",`hire`="'.$hire.'",`status`="'.$status.'" WHERE `id`="'.$id.'"';
		$result=mysqli_query($conn,$sql);
		if($result>0){
