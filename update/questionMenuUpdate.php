...
include('../db.php');
$sentId=$_GET['id'];
 $sql='SELECT * FROM `questionMenu` WHERE `id`="'.$sentId.'"';
...
<!-- <?php
	if(isset($_POST['submit'])){
		...
		$text=$_POST['text'];
		$status=$_POST['status'];
		$sql='UPDATE `questionMenu` SET `lang`="'.$lang.'",`title`="'.$title.'",`text`="'.$text.'",`status`="'.$status.'" WHERE `id`="'.$id.'"';
		$result=mysqli_query($conn,$sql);
		if($result>0){
		 	header("Location: ../questionMenu.php");
		}
	}
?> -->
</body>
