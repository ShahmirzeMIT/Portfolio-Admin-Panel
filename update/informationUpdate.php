<?php include('../db.php');
$sentId=$_GET['id'];
$sql='SELECT * FROM `information` WHERE `id`="'.$sentId.'"';
...
<form class="wd30" method="post">
  ...
  <div class="mb-3 ">
    ...
    <input type="text" class="form-control" id="exampleInputPassword1"  value="<?=$inf?>"name="inf">
  </div>
  ...
  <div class="text-center col">
	...
	<button type="submit" name="submit" class="btn btn-primary">Submit</button> <button type="submit" class="btn btn-danger"><a href="../information.php" class="text-light text-decoration-none">Go Back</a></button></div>
</form>
<?php
...
if(isset($_POST['submit'])){
	...
	$status=$_POST['status'];
	echo $id;
	$sql='UPDATE `information` SET `lang`="'.$lang.'",`target`="'.$target.'",`inf`="'.$inf.'",`status`="'.$status.'" WHERE `id`="'.$id.'"';
	$result=mysqli_query($conn,$sql);
	if($result>0){
