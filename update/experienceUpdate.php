...
<form class="wd30" method="post">
  ...
  <div class="mb-3">
    ...
    <input type="text" class="form-control" id="exampleInputPassword1" value="<?=$workName?>" name="workName">
  </div>
  <div class="mb-3 ">
...
if(isset($_POST['submit'])){
	...
	$year=$_POST['year'];
	$text=$_POST['text'];
	$sql='UPDATE `education` SET `lang`="'.$lang.'",`workName`="'.$workName.'",`year`="'.$year.'",`text`="'.$text.'" WHERE `id`="'.$id.'"';
	$result=mysqli_query($conn,$sql);
	if($result>0){
